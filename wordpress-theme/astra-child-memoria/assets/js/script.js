...
function abrirModal(dato) {
    ...
    // Link al perfil individual (WordPress)
    const linkProfile = document.getElementById('modal-link-profile');
    if (linkProfile) {
        if (dato.url) {
            linkProfile.href = dato.url;
            linkProfile.style.display = '';
        } else {
...
/* ====== Galería / Grilla con buscador ====== */

const GALLERY_PAGE_SIZE = 10;
let galeriaFiltrados = [];
let galeriaPaginaActual = 0;

function crearCardGaleria(dato) {
    const card = document.createElement('div');
    card.className = 'gallery-card';
    card.addEventListener('click', () => abrirModal(dato));

    const nombreLimpio = dato.nombre.replace(/\s*\(.*?\)\s*$/, '');

    let imgHTML;
    if (dato.imagen && dato.imagen.trim() !== '') {
        imgHTML = `<img class="gallery-card-img" src="${dato.imagen}" alt="${nombreLimpio}" onerror="this.outerHTML='<div class=\'gallery-card-placeholder\'>&#9900;</div>'">`;
    } else {
        imgHTML = '<div class="gallery-card-placeholder">&#9900;</div>';
    }

    card.innerHTML = `
        ${imgHTML}
        <div class="gallery-card-body">
            <div class="gallery-card-name">${nombreLimpio}</div>
            <div class="gallery-card-date">${dato.fecha}</div>
        </div>
    `;
    return card;
}
...
function totalPaginas() {
    return Math.max(1, Math.ceil(galeriaFiltrados.length / GALLERY_PAGE_SIZE));
}
...
function mostrarPagina(pagina) {
    ...
    galeriaPaginaActual = Math.max(0, Math.min(pagina, totalPaginas() - 1));
    const desde = galeriaPaginaActual * GALLERY_PAGE_SIZE;
    const hasta = Math.min(desde + GALLERY_PAGE_SIZE, galeriaFiltrados.length);

    grid.innerHTML = '';
    for (let i = desde; i < hasta; i++) {
        grid.appendChild(crearCardGaleria(galeriaFiltrados[i].dato));
    }
...
function actualizarPaginacion() {
    ...
    if (paginationContainer) {
        paginationContainer.style.display = galeriaFiltrados.length > 0 ? 'flex' : 'none';
    }
    if (btnPrev) btnPrev.disabled = galeriaPaginaActual === 0;
    if (btnNext) btnNext.disabled = galeriaPaginaActual >= totalPaginas() - 1;
    if (pageInfo) pageInfo.textContent = `${galeriaPaginaActual + 1} / ${totalPaginas()}`;
}
...
function renderizarGaleria(filtro = '') {
    ...
    galeriaFiltrados = datosMemoria
        .map((d, i) => ({ dato: d, index: i }))
        .filter(({ dato }) => dato.nombre.toLowerCase().includes(termino));

    mostrarPagina(0);

    if (noResults) noResults.style.display = galeriaFiltrados.length === 0 ? 'block' : 'none';
    if (countEl) {
        countEl.textContent = termino
            ? `Mostrando ${galeriaFiltrados.length} de ${datosMemoria.length}`
            : `${datosMemoria.length} personas registradas`;
    }
}
...
function configurarBuscador() {
    ...
    input.addEventListener('input', function() {
        ...
        timeout = setTimeout(() => renderizarGaleria(this.value), 200);
    });

    const btnPrev = document.getElementById('gallery-prev');
    const btnNext = document.getElementById('gallery-next');
    if (btnPrev) btnPrev.addEventListener('click', () => mostrarPagina(galeriaPaginaActual - 1));
    if (btnNext) btnNext.addEventListener('click', () => mostrarPagina(galeriaPaginaActual + 1));
}
